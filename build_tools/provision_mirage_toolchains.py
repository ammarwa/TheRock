...
def ensure_cargo(toolchain_dir: Path) -> Path:
...
def write_wrapper(
    wrapper: Path, node_bin: Path, cargo_bin: Path, real_cargo: Path
) -> None:
...
def main(argv: list[str]) -> int:
    ...
    cargo = ensure_cargo(toolchain_dir)
    node_bin = ensure_node(toolchain_dir, args.node_version)

    write_wrapper(args.wrapper.resolve(), node_bin, cargo.parent, cargo)
    return 0
